(() => {
  ...
  const EXPECTED_DIAGNOSTIC_VERSION = 'anastasia-rozalina-diagnostics-v1';
  ...
  function resolveStudent() {
    ...
    let stored = '';
    try { stored = normalizeStudentId(window.localStorage.getItem(ACTIVE_STUDENT_KEY)); } catch {}
    const fallback = normalizeStudentId(config.student?.id) || normalizeStudentId(students[0]?.id);
    const id = byStudentId.has(requested) ? requested : (byStudentId.has(stored) ? stored : fallback);
    const selected = byStudentId.get(id) || students[0] || {};
    try { window.localStorage.setItem(ACTIVE_STUDENT_KEY, normalizeStudentId(selected?.id)); } catch {}
    return selected;
  }

  const student = resolveStudent();
  const studentId = normalizeStudentId(student.id);
  const studentName = String(student.nameRu || student.nameEn || studentId || 'Ученица');
  ...
  let lastReport = { studentId, studentName, startedAt: null, checks: [], health: null, directRows: [], errors: [] };
  ...
  function renderStudentSwitcher() {
    ...
    studentSwitcherEl.innerHTML = students.map((item) => {
      const id = normalizeStudentId(item?.id);
      const active = id === studentId;
      const name = String(item?.nameRu || item?.nameEn || id);
      return `<button type="button" class="student-btn${active ? ' active' : ''}" data-student-id="${esc(id)}" aria-pressed="${active ? 'true' : 'false'}">${esc(name)}</button>`;
    }).join('');
    ...
    studentSwitcherEl.addEventListener('click', (event) => {
      ...
      try { window.localStorage.setItem(ACTIVE_STUDENT_KEY, nextId); } catch {}
  ...
  function renderConfig() {
    ...
    addKV(configInfoEl, 'Уровень', student.level || config.course?.level || '—');
    addKV(configInfoEl, 'Учебник', [student.textbook, student.textbookEdition].filter(Boolean).join(' · ') || '—');
    ...
    addKV(configInfoEl, 'Edge Function', FUNCTION_NAME, true);
  ...
  function resetChecks() {
    checksEl.innerHTML = '';
    lastReport = { studentId, studentName, startedAt: new Date().toISOString(), checks: [], health: null, directRows: [], errors: [] };
  }
  ...
  function explainFunctionFailure(result) {
    ...
    if (result?.status === 404) return `Edge Function ${FUNCTION_NAME} не найдена или не задеплоена.`;
    if (result?.status === 401 && /Unauthorized diagnostics request/i.test(message)) {
      ...
      return `Edge Function отвечает, но не принимает публичный ключ из config.js.${version} Замени и заново Deploy файл функции из архива.`;
    }
    if (result?.status === 401 && /Unauthorized/i.test(message)) return `Edge Function отклонила авторизацию. Проверь deploy ${FUNCTION_NAME} и настройки JWT.`;
    if (result?.status === 403) return message || `Диагностика запрещена для student_id=${studentId}.`;
  ...
  async function runAll() {
    ...
    resetChecks();
    setSummary('', `Проверяю подключения для ${studentName}…`);
    telegramInfoEl.innerHTML = '';

    try {
      const hasConfig = Boolean(config.supabase?.url && config.supabase?.anonKey && studentId && byStudentId.has(studentId));
      addCheck('1. config.js и профиль ученицы', hasConfig ? 'ok' : 'bad', hasConfig ? `Конфигурация загружена для ${studentName} (${studentId}).` : 'Не хватает student_id / Supabase URL / anon key.');
      ...
      const readResponse = await client
        .from(homeworkTable)
        .select('student_id,student_name,lesson_id,lesson_title,status,checked_at,submitted_at,locked_at,report_status,report_sent_at,report_error,score_correct,score_total,score_percent,answers,updated_at')
      ...
      if (readResponse.error) {
        ...
        addCheck('3. Supabase Database / чтение homework_progress', 'ok', `Доступ есть. Строк прогресса ${studentName}: ${(readResponse.data || []).length}.`);
      }
      ...
      if (!edgeResult.ok || edgeResult.data?.diagnosticVersion !== EXPECTED_DIAGNOSTIC_VERSION) {
        ...
        addCheck(`4. Supabase Edge Function ${FUNCTION_NAME}`, 'bad', detail);
        lastReport.errors.push({ stage: 'edge_function', error: detail, response: edgeResult });
      } else {
        lastReport.health = edgeResult.data;
        addCheck(`4. Supabase Edge Function ${FUNCTION_NAME}`, 'ok', `Задеплоена нужная версия: ${edgeResult.data.diagnosticVersion}.`);
        ...
        addCheck('6. Edge Function → Supabase (service role)', h.database?.ok ? 'ok' : 'bad', h.database?.ok ? `Сервер читает общие таблицы Supabase. Строк ДЗ: ${h.database.homeworkRows}.` : (h.database?.error || 'Сервер не может читать Supabase.'));
        addCheck('7. Получатель Telegram', h.recipient?.ok ? 'ok' : 'bad', h.recipient?.ok ? `Получатель для ${studentName} найден и включён в pair_telegram_recipients.` : (h.recipient?.error || 'Получатель не найден/выключен.'));
        addCheck('8. Тема Telegram', h.recipient?.ok && Number(h.recipient?.threadId) === 2 ? 'ok' : 'bad', h.recipient?.ok ? `message_thread_id=${h.recipient.threadId ?? 'NULL'}; ожидается 2.` : 'Нельзя проверить тему без настроенного получателя.');
        ...
        addCheck('10. Telegram Bot API / группа', h.telegram?.chat?.ok ? 'ok' : 'bad', h.telegram?.chat?.ok ? `Бот имеет доступ к целевой группе (${h.telegram.chat.type || 'chat'}).` : (h.telegram?.chat?.error || 'Бот не имеет доступа к целевой группе.'));

        if (Array.isArray(h.database?.suspiciousHomework) && h.database.suspiciousHomework.length) {
          addCheck('11. Состояние сохранённых ДЗ', 'warn', `Есть несогласованные записи: ${h.database.suspiciousHomework.map((item) => `${item.lessonId || 'unknown'}${item.reason ? ` — ${item.reason}` : ''}`).join('; ')}.`);
        } else {
          addCheck('11. Состояние сохранённых ДЗ', 'ok', 'Записи соответствуют state machine draft → submitted_pending_report → submitted.');
        }

        const pendingHomework = Array.isArray(h.database?.pendingHomework) ? h.database.pendingHomework : [];
        const pendingReports = Array.isArray(h.reportLog?.pendingOrFailed) ? h.reportLog.pendingOrFailed : [];
        if (pendingHomework.length || pendingReports.length) {
          const parts = [];
          if (pendingHomework.length) parts.push(`homework_progress: ${pendingHomework.map((item) => `${item.lessonId} (${item.reportStatus || 'pending'})`).join(', ')}`);
          if (pendingReports.length) parts.push(`pair_homework_reports: ${pendingReports.map((item) => `${item.lessonId} (${item.status || 'unknown'})`).join(', ')}`);
          addCheck('12. Telegram-отчёты по ДЗ', 'warn', `Есть ожидающие/ошибочные записи. ${parts.join(' · ')}`);
        } else {
          addCheck('12. Telegram-отчёты по ДЗ', 'ok', 'Зависших отчётов по выбранной ученице нет.');
        }

        if (removedProbes > 0) addCheck('13. Очистка диагностики', 'ok', `Удалено старых технических probe-записей: ${removedProbes}.`);

        telegramInfoEl.innerHTML = '';
        addKV(telegramInfoEl, 'Ученица', studentName);
        addKV(telegramInfoEl, 'student_id', studentId, true);
      ...
      if (bad.length) {
        setSummary('bad', `Найдена проблема для ${studentName}: ${bad[0].name}. Смотри первую красную строку выше.`);
      } else if (warn.length) {
        setSummary('warn', `Основные подключения ${studentName} работают, но есть предупреждение по сохранённым данным.`);
      } else {
        setSummary('ok', `Все проверенные подключения ${studentName} работают. Теперь можно отдельно проверить запись Supabase и тестовое сообщение в Telegram.`);
      }
    } catch (error) {
  ...
  async function testDatabaseWrite() {
    ...
    dbWriteResultEl.innerHTML = `<div class="summary">Проверяю полный путь сохранения ДЗ для ${esc(studentName)}…</div>`;
    ...
    try {
      const { error: insertError } = await client.from(table).insert({
        ...
        lesson_id: probeId,
        lesson_title: `Diagnostics probe · ${studentName}`,
        status: 'draft',
        answers: {},
        score_correct: null,
        score_total: null,
      ...
      dbWriteResultEl.innerHTML = `<div class="summary ok">✓ Полный путь homework_progress для ${esc(studentName)} работает: browser draft → submitted_pending_report → submitted → cleanup. Реальные ДЗ не изменялись.</div>`;
      lastReport.databaseWriteProbe = { ok: true, studentId, lessonId: probeId, stages: probe.data.stages || null };
    } catch (error) {
      const detail = formatError(error);
      dbWriteResultEl.innerHTML = `<div class="summary bad">✕ Ошибка пути homework_progress для ${esc(studentName)}: ${esc(detail)}</div>`;
      lastReport.errors.push({ stage: 'database_write_probe', error: detail, studentId, lessonId: probeId });
  ...
  async function sendTestReport() {
    ...
    sendResultEl.innerHTML = `<div class="summary">Отправляю тестовое сообщение для ${esc(studentName)}…</div>`;
    try {
      ...
      if (result.data.skipped) {
        ...
        sendResultEl.innerHTML = `<div class="summary warn">Тест для ${esc(studentName)} уже отправлялся совсем недавно. Повтори примерно через ${retry} сек.</div>`;
      } else {
        sendResultEl.innerHTML = `<div class="summary ok">✓ Telegram принял тест для ${esc(studentName)}. message_id=${esc(result.data.telegramMessageId)}; thread_id=${esc(result.data.threadId ?? 'NULL')}.</div>`;
      }
      ...
      lastReport.errors.push({ stage: 'telegram_test_send', error: detail, studentId });
    } finally {
      rawEl.textContent = JSON.stringify(lastReport, null, 2);
